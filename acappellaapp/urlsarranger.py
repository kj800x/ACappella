...
urlpatterns = patterns('acappellaapp.arrangerviews',
    ...
    url(r'^group/(?P<group_short_code>[^/]+)/song/(?P<song_short_code>[^/]+)/delete/track/(?P<track_id>[^/]+)/$', 'deletetrack', name='arrangerdeletetrack'),
    url(r'^group/(?P<group_short_code>[^/]+)/song/(?P<song_short_code>[^/]+)/delete/pdf/$', 'deletepdf', name='arrangerdeletepdf'),
)
